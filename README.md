...
- **Hide a stop from the map** — an eye next to the ✓ and the bin on every stop card (and in its detail popup) takes that stop's pin off the map without taking it out of the day: the card dims, its number badge goes hollow, and the map simply skips it. Numbers never shift — the day still reads 1, 2, 3, 4 on the cards and 1, 2, 4 on the map, so the gap is obvious and the stop it belongs to is easy to find. Everything else is untouched: the stop keeps its place, its time and its travel legs, and the route still runs through it, because the day still goes there. It lives on the stop, so it travels through export/import, the offline copy, the KML and shared rooms (a hidden stop opens unticked in the Google Maps picker, ready to tick back in). For the corner of a map too crowded to read, or a stop you're still undecided about.
